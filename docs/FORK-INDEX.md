...
The Advisor gitlink path is `plugins/advisor`, pinned to `82591078ac5d3d93b7d57c262077c4d0d62de71d` (matches `main` tip after [advisor #9](https://github.com/atebites-hub/advisor/pull/9)). Productize scrub landed. The nested Codex package inside that checkout remains `plugins/advisor/plugins/sol-advisor`. Claude `nativeAdvisor` is still unverified (fixture follow-up).
...
2. **Marketplace pin bumps only after QA playtest / smoke.** Advisor gitlink is `82591078` (matches tip; Factory QA Lane A PASS on productize #9). Nested ODW `zcode-cli` pin on plugin `main` is `cfd0de2` (matches tip). Do not bump ponytail, Superpowers, ODW, or other marketplace SHAs from this catalog. Record `not run — Lane B awaiting Jay credentials` instead of fake green.
