...
- **Advisor** — Factory @ `6d2b562ec7b29d98792d83999eea3760a1365056` (matches tip after [advisor #10](https://github.com/atebites-hub/advisor/pull/10)). ODW native-alignment docs landed; alignment still unproven until live QA.
...
The Advisor gitlink path is `plugins/advisor`, pinned to `6d2b562ec7b29d98792d83999eea3760a1365056` (matches `main` tip after [advisor #10](https://github.com/atebites-hub/advisor/pull/10)). Factory story. Alignment docs landed; alignment status still unproven until live QA. Productize scrub landed. The nested Codex package inside that checkout remains `plugins/advisor/plugins/sol-advisor`. Claude `nativeAdvisor` is still unverified (fixture follow-up).
...
5. **Upcoming (not done):** factory-policy (+ memory); linear-tracking pin path; GitNexus wrap. Brief pointers only — this catalog does not vendor them yet.
