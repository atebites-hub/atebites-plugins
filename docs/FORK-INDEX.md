...
1. **Ensure `UPSTREAM_SYNC_TOKEN` PATs have Pull requests: write** (or classic `repo`) so weekday sync can open PRs unattended — push already works. Fine-grained tokens that can push still fail `createPullRequest` (`Resource not accessible by personal access token`); ponytail sync branch pushed, PR opened manually ([#4](https://github.com/atebites-hub/ponytail/pull/4), now merged).
