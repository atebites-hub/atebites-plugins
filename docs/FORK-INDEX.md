...
"Sync loop" means a weekday workflow (or equivalent) that opens `chore: sync upstream` when upstream moved. Existing CI (`ci.yml`, `test.yml`, …) is not a sync loop.
...
2. Add `UPSTREAM.md` and the weekly weekday sync loop on every linked fork (ODW core, ponytail, sol-advisor, taskboard).
